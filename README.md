...
- **文件编码**：所有脚本以 UTF-8（无 BOM）读写，避免中文乱码。若你本地预览仍乱码，确认编辑器以 UTF-8 打开。
- **WP 区块兼容**：`publish_draft.py` 的 MD→HTML 转换已修复常见坑：
  - `0xN` 小标题**必须严格以 `0x` 开头**才转 `<h3>`（避免把"8月…"误判为标题）
  - WP 短代码（如 `[postsbox post_id="X"]`）原样保留，不包 `<p>`，避免古腾堡编辑器解析异常
